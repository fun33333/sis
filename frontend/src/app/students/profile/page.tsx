...
export default function StudentProfilePage() {
  ...
  const gradeBySubject = useMemo(() => {
    const subjects = ["Science","Maths","English","Physics","Arts"]
    return subjects.map((s, idx) => ({ name: s, value: Math.max(50, Math.min(100, student.averageScore - idx * 3)), fill: idx % 2 === 0 ? '#0b6b58' : '#16a34a' }))
  }, [student])
        ...
        <aside className="lg:col-span-3">
          <Card>
            <CardContent>
              ...
              <Select value={student.studentId} onValueChange={(v) => router.push(`/students/profile?studentId=${v}`)}>
                ...
                <SelectContent>
                  {mockStudents.slice(0, 200).map((st) => (
          ...
          <div className="mt-4">
            <div className="bg-white rounded shadow overflow-hidden">
              <img src={`https://images.unsplash.com/photo-1544005313-94ddf0286df2?auto=format&fit=crop&w=800&q=60`} alt={student.name} className="w-full h-96 object-cover" />
            </div>
          </div>
        </aside>
                ...
                <table className="w-full table-fixed border-collapse">
                  <tbody>
                    <tr className="border-t">
                      <td className="w-1/3 p-4 font-semibold">Student ID</td>
                      <td className="p-4">{student.studentId.replace(/^STU/, '')}</td>
                    </tr>
                    <tr className="border-t">
                    ...
                    <tr className="border-t">
                      <td className="p-4 font-semibold">Grade</td>
                      <td className="p-4">{student.grade}</td>
                    </tr>
                    <tr className="border-t">
                      <td className="p-4 font-semibold">Age</td>
                      <td className="p-4">{6 + (student.averageScore % 13)}</td>
                    </tr>
                    <tr className="border-t">
                      <td className="p-4 font-semibold">Gender</td>
                      <td className="p-4">{student.gender}</td>
                    </tr>
                    <tr className="border-t">
                      <td className="p-4 font-semibold">Nationality</td>
                      <td className="p-4">Local</td>
                    </tr>
                  </tbody>
                </table>
              </div>
